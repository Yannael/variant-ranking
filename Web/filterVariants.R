createFilterVariant<-function(input,output,session) {
  
  if (!file.exists("filterVariant.csv")) {
    data<-rbind(c("All",""))
    colnames(data)<-c("Name","SQL")
    write.table(file="filterVariant.csv",data,row.names=F,sep="\t")
  }
  ...
  observe({
    ...
    isolate({
      if ((length(input$filterVariantNameSave)>0) & length(input$filterVariantQueryBuilderSQL)>0) {
        ...
        write.table(file=("filterVariant.csv"),data,append=T,row.names=F,col.names=F,sep="\t")
  ...
  observe({
    ...
    isolate({
      if (length(input$filterVariantSelectDelete)>0) {
        ...
        colnames(data)<-c("Name","SQL")
        write.table(file=("filterVariantSpec.csv"),data,row.names=F,col.names=T,sep="\t")
